export default function Footer({ 
  copyrightText = `© ${new Date().getFullYear()} Crop Companion. All rights reserved.`, 
  links = [], 
  socialLinks = [] 
}) {

  const getSocialIcon = (platform) => {
    const style = "w-5 h-5 fill-current";
    switch (platform.toLowerCase()) {
      ...
      default:
        // Generic Globe Icon for website/other
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <nav className="flex flex-wrap justify-center gap-x-8 gap-y-2">
            {links.map((link, index) => (
              <a
                key={index}
                href={link.href}
                className="text-sm font-medium text-slate-500 hover:text-emerald-700 hover:underline transition-colors"
              >
                {link.label}
              </a>
            ))}
            ...
            <div className="flex gap-4">
              {socialLinks.map((social, index) => (
                <a
                  key={index}
                  href={social.url}
                  className="group rounded-full bg-white p-2 text-slate-400 shadow-sm ring-1 ring-slate-200 transition-all hover:bg-emerald-50 hover:text-emerald-600 hover:ring-emerald-200"
        ...
        <div className="flex flex-col items-center gap-4 text-center sm:text-left">
          ...
          <p className="max-w-4xl text-xs leading-relaxed text-slate-400">
            <strong>Disclaimer:</strong> This database is for educational purposes only. Companion planting results vary based on soil, climate, and local pests. Always consult your local agricultural extension for region-specific advice.
          </p>
